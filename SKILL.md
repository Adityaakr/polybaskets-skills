...
Skill pack for AI agents to use PolyBaskets — an ETF-style prediction market aggregator on Vara Network. Season 3 economy: agents claim free CHIP tokens hourly (500 base, +10/UTC-day streak, cap 600 at day 11), bet on prediction baskets, and collect payouts when markets resolve. Native VARA freebet balances are handled separately through FreebetLedger. Gas vouchers use an hourly-tranche model — a single batched POST registers the required programs and adds +500 VARA per hourly top-up (1 funded POST/hour/wallet).
...
#    missing one of the required programs, OR known balance is below 10 VARA and
...
if [ "$VOUCHER_ID" = "null" ] || [ "$HAS_ALL_PROGRAMS" != "true" ] || { [ "$NEED_TOP_UP" = "true" ] && [ "$CAN_TOP_UP" = "true" ]; }; then
  # Single batched POST — required programs registered + 500 VARA added only
...
- Use `basket-freebet/SKILL.md` when the user means a non-withdrawable native VARA freebet balance in `FreebetLedger`.
- This is not the CHIP lane. The agent checks `FreebetLedger/BalanceOf`, finds or creates `asset_kind: "Vara"` baskets, computes `index_at_creation_bps`, calls `FreebetLedger/SpendFreebet`, verifies `BasketMarket/GetFreebetPositions`, and claims through `BasketMarket/Claim`.
- Freebet principal returns to `FreebetLedger` on claim; only profit above stake is paid to the wallet.
...
3. **Use `--account agent --voucher $VOUCHER_ID`** for any command that writes to the blockchain (Claim, Approve, PlaceBet, SpendFreebet). The voucher pays for gas. Do NOT use `--account` or `--voucher` for read-only queries.
...
15. **Default public agent path is CHIP.** Use CHIP (BetLane) and create `asset_kind: "Bet"` baskets unless the user explicitly asks for native VARA freebet. For native freebet, use only `asset_kind: "Vara"` baskets and first confirm `BasketMarket/IsVaraEnabled=true`.
