...
- `end_timestamp` — the item's Polymarket `endDate` in **Unix milliseconds** (e.g. `Date.parse(market.endDate)`). **Required** — the contract stores it and the bet-quote-service uses it to decide if a basket is still bettable. Omitting it defaults to `0`, which makes the basket immediately fail the bet cutoff (see below), so the quote service will report it "not active".

**Bet cutoff:** `GetConfig.bet_cutoff_ms` (currently `300000` = 5 min). Every item's `end_timestamp` must be **more than 5 minutes in the future** at bet time, or `CreateBasket`/`PlaceBet` panics with `BetCutoffReached`. For ultra-short interval markets, create and bet while at least ~6 minutes remain.
